...
class Proximity(nn.Module):
    def __init__(
        ...
        if distance_metric not in {"l2", "tropical"}:
            raise ValueError(f"Unsupported metric {distance_metric!r}. Choose 'l2' or 'tropical'.")
        self.distance_metric = distance_metric

        # initialize class centers on the correct device
    ...
    def forward(self, x: torch.Tensor, labels: torch.LongTensor) -> torch.Tensor:
        ...
        returns: mean distance according to self.distance_metric
        """
        B, C = x.size(0), self.num_classes

        if self.distance_metric == "l2":
            # ---- L2 distance ----
            x_norm2 = x.pow(2).sum(dim=1, keepdim=True)                     # [B,1]
            c_norm2 = self.centers.pow(2).sum(dim=1, keepdim=True).T        # [1,C]

            distmat = x_norm2 + c_norm2
            distmat = distmat.addmm(x, self.centers.t(), beta=1, alpha=-2)

            # mask for true classes
            class_range = torch.arange(C, device=x.device)
            mask = labels.view(-1,1) == class_range.view(1,-1)            # [B,C] bool

            target_dists = distmat[mask]                                   # [B]
            target_dists = target_dists.clamp(min=1e-12, max=1e12)
            return target_dists.mean()

        else:
            # ---- Tropical symmetric distance ----
            centers_labels = self.centers[labels]                           # [B, F]
            diff = x - centers_labels                                       # [B, F]
            min_diff, _ = diff.min(dim=1)                                   # [B]
            max_diff, _ = diff.max(dim=1)                                   # [B]
            tropical_dists = max_diff - min_diff                            # [B]
...
class ConProximity(nn.Module):
    def __init__(
        ...
        # initialize centers directly on the target device
        self.centers = nn.Parameter(
            torch.randn(num_classes, feat_dim, device=device)
        )
    ...
    def forward(self, x: torch.Tensor, labels: torch.LongTensor) -> torch.Tensor:
        ...
        x:      (B, F) feature vectors
        labels: (B,) integer class labels in [0..num_classes-1]
        returns: mean distance over all non-target centers,
                 using either L2 or tropical symmetric metric.
        """
        B, C = x.size(0), self.num_classes

        # build boolean mask for true classes
        class_range = torch.arange(C, device=x.device)
        mask_target = labels.view(-1, 1) == class_range.view(1, -1)  # [B, C] bool

        if self.distance_metric == "l2":
            # === L2 distances to all centers ===
            x_norm2 = x.pow(2).sum(dim=1, keepdim=True)                # [B,1]
            c_norm2 = self.centers.pow(2).sum(dim=1, keepdim=True).T   # [1,C]
            distmat = x_norm2 + c_norm2
            distmat = distmat.addmm(x, self.centers.t(), beta=1, alpha=-2)  # [B,C]

            # select all non-target distances
            non_target_dists = distmat[~mask_target]   # [B*(C-1)]
            non_target_dists = non_target_dists.clamp(min=1e-12, max=1e12)
            return non_target_dists.mean()

        else:
            # === Tropical symmetric distances to all centers ===
            # x: [B, F] -> [B, 1, F]; centers: [C, F] -> [1, C, F]
            diff = x.unsqueeze(1) - self.centers.unsqueeze(0)      # [B, C, F]

            # tropical distance: min(diff) - max(diff) over feature dim
            diff_min = diff.min(dim=2).values                     # [B, C]
            diff_max = diff.max(dim=2).values                     # [B, C]
            trop_mat = diff_max - diff_min                        # [B, C]

            # select non-target tropical distances
            non_target_trop = trop_mat[~mask_target]               # [B*(C-1)]
